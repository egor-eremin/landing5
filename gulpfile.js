...
const __release = process.title === 'gulp build:release' || process.title === 'gulp build:none';
const __none = process.title === 'gulp build:none';
const configWebpack = __none ? require('./webpack.none.config') : ( __release ? require('./webpack.release.config') : require('./webpack.dev.config') );
...
function scriptscache() {
    return src(siteconfig.build.links.source.assets.js + '/**/*.*')
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.license.lls1', '__CONFIG_GLOBAL.nonecompany.license.lls1'))) // обезличивание: смена лицензии брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.license.lls2', '__CONFIG_GLOBAL.nonecompany.license.lls2'))) // обезличивание: смена лицензии брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.license.lls3', '__CONFIG_GLOBAL.nonecompany.license.lls3'))) // обезличивание: смена лицензии брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.title', '__CONFIG_GLOBAL.nonecompany.title'))) // обезличивание: смена название брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.adres', '__CONFIG_GLOBAL.nonecompany.adres'))) // обезличивание: смена адреса брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.email', '__CONFIG_GLOBAL.nonecompany.email'))) // обезличивание: смена email брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.phone', '__CONFIG_GLOBAL.nonecompany.phone'))) // обезличивание: смена телефон брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.url', '__CONFIG_GLOBAL.nonecompany.url'))) // обезличивание: смена url брокера
        .pipe(gulpif(__none, rep('__CONFIG_GLOBAL.companyData.domain', '__CONFIG_GLOBAL.nonecompany.domain'))) // обезличивание: смена домена брокера
        .pipe(dest(siteconfig.build.links.build.assets.core + '/cache/'))
}
...
function scriptscacheclean() {
    return del([siteconfig.build.links.build.assets.core + '/cache'], {force:true});
}
...
function buildsender() {
    ...
    return src(siteconfig.build.links.source.resources + '/' + siteconfig.build.links.source.sender.title)
        .pipe(rename(function(path) {
            path.basename = (__none) ? path.basename : newname; // если обезличивание то не переименовывать
...
function pg() {
    return src([siteconfig.build.links.source.pages + '/**/*.pug'])
        ...
        .pipe(injectdata(function(file) {
            try {
                if (fs.existsSync(globalConfig)) {
                    ...
                    // если сборка обезличина то делать замену на данные по обезличиванию
                    if (__none) {
                        configFile.companyData.title = configFile.nonecompany.title;
                        configFile.companyData.adres = configFile.nonecompany.adres;
                        configFile.companyData.phone = configFile.nonecompany.phone;
                        configFile.companyData.email = configFile.nonecompany.email;
                    }
                    return configFile;
                }
            } catch (err) {
        ...
        .pipe(gulpif(__none, rep(siteconfig.companyData.license.lls1, siteconfig.nonecompany.license.lls1))) // обезличивание: смена лицензии брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.license.lls2, siteconfig.nonecompany.license.lls2))) // обезличивание: смена лицензии брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.license.lls3, siteconfig.nonecompany.license.lls3))) // обезличивание: смена лицензии брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.title, siteconfig.nonecompany.title))) // обезличивание: смена название брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.adres, siteconfig.nonecompany.adres))) // обезличивание: смена адреса брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.email, siteconfig.nonecompany.email))) // обезличивание: смена email брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.phone, siteconfig.nonecompany.phone))) // обезличивание: смена телефона брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.url, siteconfig.nonecompany.url))) // обезличивание: смена url брокера
        .pipe(gulpif(__none, rep(siteconfig.companyData.domain, siteconfig.nonecompany.domain))) // обезличивание: смена domain брокера
        ...
        .pipe(through.obj(function(file, encoding, callback) {
            ...
            if (file.isBuffer()) {
                let result = file.contents.toString().split('\n').map(function(st, i) {
                    ...
                    return (__none) ? st.split(siteconfig.companyData.title).join(siteconfig.nonecompany.title) // обезличивание: смена название брокера
                            .split(siteconfig.companyData.email).join(siteconfig.nonecompany.email) // обезличивание: смена email брокера
                            .split(siteconfig.companyData.phone).join(siteconfig.nonecompany.phone) // обезличивание: смена phone брокера
                            .split(siteconfig.companyData.adres).join(siteconfig.nonecompany.adres) // обезличивание: смена adres брокера
                            : st ;
                }).join('\r\n'); // закрываем контент
...
exports.scriptscache = scriptscache;
exports.scriptscacheclean = scriptscacheclean;
...
exports['build:none'] = series(cleanbuild, jsonldeditor, jsoncontent, buildcopy, buildres, buildsender, buildrssphp, pg, scriptscache, scripts, scriptscacheclean, icons, stylesbuild, askAllImages, replaceAllImagesHTML, replaceAllImagesCSS, replaceAllImagesJS, createsitemap, createrobots);
